...
class dbobject extends validation
{
	...
	public function db_query($sql, $object = true)
	{
		...
		file_put_contents('lo.txt', $sql, FILE_APPEND);
		$cnx = new dbcnx();
		$this->myconn = $cnx->connect();
...
*/
	...
	{
		$whrClause = $selectClause = $label = "";
		$ret_val = $retValue = array();
		/////////////////////////////////////////////////////////////////
		...
		if ($ret_val_arr == "*") {
			...
			foreach ($result as  $key => $roww) {
				...
				$selectClause .= $roww['Field'] . $delimiter;
	...
	public function CORS($data)
    {
		$ip = $_SERVER['REMOTE_ADDR'];
        $app_local_link = $this->getitemlabel("parameter", "parameter_name", "site_local_url", "parameter_value").$data;
        $app_live_link = $this->getitemlabel("parameter", "parameter_name", "site_live_url", "parameter_value").$data;
